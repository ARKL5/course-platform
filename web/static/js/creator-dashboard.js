...
class CreatorDashboard {
    ...
    async uploadCoverImage(courseId, file) {
        ...
        await this.updateCourseCover(courseId, fileUrl);
        
        return result;
    }

    // 更新课程封面
    async updateCourseCover(courseId, coverImageUrl) {
        const token = localStorage.getItem('authToken');
        const updateData = {
        ...
        if (!response.ok) {
            const error = await response.json();
            throw new Error(error.message || '更新课程封面失败');
        }

        console.log('课程封面更新成功:', coverImageUrl);
